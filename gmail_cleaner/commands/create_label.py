...
def create_label(
    path: str = typer.Argument(
        ...,
        help='Path to JSONL file of labels, or "-" for stdin.',
    ),
) -> None:
    ...
    label_dicts = _parse_jsonl(path)
    try:
        created = labels.create_labels(creds, label_dicts)
    except labels.CreateLabelsError as exc:
        _print_created(exc.created)
        typer.echo(f'create-label failed: {exc.__cause__}', err=True)
